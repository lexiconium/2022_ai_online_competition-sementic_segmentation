...
# compatibility
class SegformerDualHeadSegmentation(SegformerPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(
        self,
        classifier_pixel_values: Optional[torch.FloatTensor] = None,
        segmenter_pixel_values: Optional[torch.FloatTensor] = None,
        classifier_labels: Optional[torch.LongTensor] = None,
        segmenter_labels: Optional[torch.FloatTensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None
    ):
        ...
        segmenter_loss = None
        if segmenter_labels is not None:
            if self.config.num_labels == 1:
                raise ValueError("The number of labels should be greater than one")
            else:
                # upsample logits to the images' original size
                upsampled_logits = nn.functional.interpolate(
                    segmenter_logits, size=segmenter_labels.shape[-2:], mode="bilinear", align_corners=False
                )
                loss_fct = nn.CrossEntropyLoss(
                    weight=torch.Tensor([1, 2, 1, 1, 1]), ignore_index=self.config.semantic_loss_ignore_index
                )
                segmenter_loss = loss_fct(upsampled_logits, segmenter_labels)
        ...
        return DualHeadSemanticSegmenterOutput(
            ...
            classifier_logits=classifier_logits,
            logits=segmenter_logits,
            classifier_hidden_states=classifier_outputs.hidden_states,
            hidden_states=segmenter_outputs.hidden_states if output_hidden_states else None
        )
